        ...
        <PageHeaderLayout title="零部件详情">
          <Card bordered={false}>
            <DescriptionList size="large" title="基础信息" style={{ marginBottom: 32 }}>
              ...
              <Description term="状态">
                <Badge status={statusMap[profile.teil.status]} text={status[profile.teil.status]} />
              </Description>
              ...
              <Description term="Abgasstufe">
                {profile.teil.abgasstufe ? profile.teil.abgasstufe.name : null}
              </Description>
              <Description term="Ersteinsatz Modell">
                {profile.teil.erstModell ? profile.teil.erstModell.name : null}
              </Description>
              <Description term="Ersteinsatz Aggregate">
                {profile.teil.erstAggregate ? profile.teil.erstAggregate.name : null}
              </Description>
              <Description term="Fzg. Modell">{modellsResult}</Description>
              <Description term="Fzg. Aggregate">{aggregatesResult}</Description>
              <Description term="FOP/MOP">
                {profile.teil.fop ? profile.teil.fop.name : null}
              </Description>
              <Description term="录入时间">
                {moment(profile.teil.inTime).format('YYYY-MM-DD HH:mm:ss')}
              </Description>
              <Description term="更新时间">
                {moment(profile.teil.updateTime).format('YYYY-MM-DD HH:mm:ss')}
              </Description>
            </DescriptionList>
            <Divider style={{ marginBottom: 32 }} />
            <DescriptionList size="large" title="研发信息" style={{ marginBottom: 32 }}>
              <Description term="询价资料时间">
                {momentDate(profile.teilSchedule.anfragedatenTime)}
              </Description>
              <Description term="采购定点计划时间">
                {momentDate(profile.teilSchedule.cscSollTime)}
              </Description>
              <Description term="采购定点实际时间">
                {momentDate(profile.teilSchedule.cscIstTime)}
              </Description>
              <Description term="项目启动会计划时间">
                {momentDate(profile.teilSchedule.kickoffSollTime)}
              </Description>
              <Description term="项目启动会实际时间">
                {momentDate(profile.teilSchedule.kickoffIstTime)}
              </Description>
              <Description term="PAB编辑发送时间">
                {momentDate(profile.teilSchedule.pabEditSendTime)}
              </Description>
              <Description term="PAB系统流转完成时间">
                {momentDate(profile.teilSchedule.pabFlowFinishTime)}
              </Description>
              <Description term="PAB费用反馈的时间">
                {momentDate(profile.teilSchedule.pabCostFeedbackTime)}
              </Description>
              <Description term="B-F计划时间">
                {momentDate(profile.teilSchedule.bfSollTime)}
              </Description>
              <Description term="B-F实际时间">
                {momentDate(profile.teilSchedule.bfIstTime)}
              </Description>
              <Description term="首模件实际时间">
                {momentDate(profile.teilSchedule.firstTryoutIstTime)}
              </Description>
              <Description term="批量件计划时间">
                {momentDate(profile.teilSchedule.otsSollTime)}
              </Description>
              <Description term="批量件实际时间">
                {momentDate(profile.teilSchedule.otsIstTime)}
              </Description>
              ...
              <Description term="BMG计划时间">
                {momentDate(profile.teilSchedule.bmgSollTime)}
              </Description>
              <Description term="BMG实际时间">
                {momentDate(profile.teilSchedule.bmgIstTime)}
              </Description>
              <Description term="BMG-EMP计划时间">
                {momentDate(profile.teilSchedule.bmgEmpSollTime)}
              </Description>
              <Description term="BMG-EMP实际时间">
                {momentDate(profile.teilSchedule.bmgEmpIstTime)}
              </Description>
              <Description term="BMG-ONLINE编号">{profile.teilSchedule.bmgOnlineNum}</Description>
              <Description term="FE54计划时间">
                {momentDate(profile.teilSchedule.fe54SollTime)}
              </Description>
              <Description term="FE54实际时间">
                {momentDate(profile.teilSchedule.fe54IstTime)}
              </Description>
              ...
              <Description term="更新时间">
                {momentTime(profile.teilSchedule.updateTime)}
              </Description>
            </DescriptionList>
            <Divider style={{ marginBottom: 32 }} />
            <DescriptionList size="large" title="试验信息" style={{ marginBottom: 32 }}>
              <Description term="尺寸计划送样时间">
                {momentDate(profile.teilTest.sizeSollTime)}
              </Description>
              <Description term="尺寸实际送样时间">
                {momentDate(profile.teilTest.sizeIstTime)}
              </Description>
              <Description term="尺寸实验实际完成时间">
                {momentDate(profile.teilTest.sizeTestIstTime)}
              </Description>
              <Description term="尺寸实验报告">{sizeTestReport}</Description>
              <Description term="材料计划送样时间">
                {momentDate(profile.teilTest.materialSollTime)}
              </Description>
              <Description term="材料实际送样时间">
                {momentDate(profile.teilTest.materialIstTime)}
              </Description>
              <Description term="材料实验实际完成时间">
                {momentDate(profile.teilTest.materialTestIstTime)}
              </Description>
              <Description term="材料实验报告">{materialTestReport}</Description>
              <Description term="供应商自检实验完成时间">
                {momentDate(profile.teilTest.supplierTestIstTime)}
              </Description>
              <Description term="供应商自检实验报告">{supplierTestReport}</Description>
              <Description term="零件寄WOB做性能时间">
                {momentDate(profile.teilTest.sendWobTime)}
              </Description>
              <Description term="WOB实验完成时间">
                {momentDate(profile.teilTest.wobTestCompleteTime)}
              </Description>
              <Description term="WOB实验报告">{wobTestReport}</Description>
              <Description term="跑车耐久实验搭载车辆信息">
                {profile.teilTest.endureTestCarInfo}
              </Description>
